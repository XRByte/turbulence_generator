...
class TurbGen
{
	...
	// ******************************************************
      public:
	int init_single_realisation(const double ndim, const double L[3], const double k_min, const double k_max, const int spect_form,
				    const double power_law_exp, const double angles_exp, const double sol_weight, const int random_seed)
	{
		return init_single_realisation(ndim, L, k_min, k_max, k_max, spect_form, power_law_exp, power_law_exp, angles_exp, sol_weight, random_seed);
	}; // init_single_realisation (overloaded)
	// ******************************************************
      public:
	int init_single_realisation(const double ndim, const double L[3], const double k_min, const double k_mid, const double k_max, const int spect_form,
				    const double power_law_exp, const double power_law_exp_2, const double angles_exp, const double sol_weight,
				    const int random_seed)
	{
		...
		this->kmid = (k_mid + DBL_EPSILON) * 2 * M_PI / L[X]; // Middle  wavenumber >~  k_mid * 2pi / Lx (only if spect_form =
		...
		this->power_law_exp_2 = power_law_exp_2;	      // power-law amplitude exponent 2 (only if spect_form =
	...
	virtual int init_driving(std::string parameter_file, const double time)
	{
		...
		ret = read_from_parameter_file("ampl_factor",
		...
		t_decay = L[X] / k_driv / velocity;		 // Auto-correlation time, t_turb = Lx / k_driv /
		...
		seed = random_seed;				 // copy original seed into local seed;
								 ...
								 // generator is called
		const double ampl_coeff = 0.15;			 // This is the default amplitude coefficient ('ampl_coeff') that
		...
		if (PE == 0) {
			...
			outfilestream << std::setw(24) << "#01_time" << std::setw(24) << "#02_time_in_t_turb" << std::setw(24) << "#03_ampl_factor_x"
				      << std::setw(24) << "#04_ampl_factor_y" << std::setw(24) << "#05_ampl_factor_z" << std::setw(24) << "#06_v_turb_prev_x"
				      << std::setw(24) << "#07_v_turb_prev_y" << std::setw(24) << "#08_v_turb_prev_z" << std::endl;
			outfilestream.close();
			outfilestream.clear();
		}
		if (verbose)
	...
	// ******************************************************
      public:
	virtual bool check_for_update(const double time, const std::vector<double> &v_turb)
	{
		// ******************************************************
		...
		if ((ampl_auto_adjust == 1) && (v_turb[X] > 0)) {
			...
			if (time > 0.1 * t_decay) { // auto-adjustment start time (start at t >
				...
				TurbGen_printf_raw("\n");
				for (int d = 0; d < ncmp; d++)
					ampl_factor[d] = ampl_factor_adjusted[d]; // set new (adjusted) ampl_factor
			}
	...
	// ******************************************************
      public:
	bool write_to_evol_file(const double time, const double ampl_factor[], const std::vector<double> v_turb)
	{
		if (verbose > 1)
			TurbGen_printf("Writing to evolution file: time = %e, time/t_turb = %-7.2f\n", time, time / t_decay);
		std::ofstream outfilestream(evolfile.c_str(), std::ios::app);
		outfilestream.precision(16);
		outfilestream << std::scientific << std::setw(24) << time << std::setw(24) << time / t_decay << std::setw(24) << pow(ampl_factor[X], 1.0 / 1.5)
			      << std::setw(24) << pow(ampl_factor[Y], 1.0 / 1.5) << std::setw(24) << pow(ampl_factor[Z], 1.0 / 1.5) << std::setw(24)
			      << v_turb[X] << std::setw(24) << v_turb[Y] << std::setw(24) << v_turb[Z] << std::endl;
		outfilestream.close();
		outfilestream.clear();
		return true;
	}; // write_to_evol_file
	...
	{
		if (verbose > 1)
			TurbGen_printf("Reading from evolution file: time = %e, time/t_turb = %-7.2f\n", time, time / t_decay);
		...
		if (infilestream.is_open()) {
			...
			for (int i = lines.size() - 1; i >= 0; i--) {
				...
				if (time_in_file >= time - 1.1 * dt) { // search backwards and update ampl_factor with
								       // last valid time
					for (unsigned int d = 0; d < ncmp; d++)
						ampl_factor[d] = ampl_factor_in_file[d];
				} else
	...
	void get_turb_vector(const double pos[], double v[])
	{
		...
		for (int m = 0; m < nmodes; m++) {
			ampl[m] = 2.0 * sol_weight_norm * this->ampl[m]; // pre-compute amplitude including normalisation
									 // factors
		...
		for (int m = 0; m < nmodes; m++) {
			...
			if (ncmp > 1)
				v[Y] += ampl[m] * (aka[Y][m] * real - akb[Y][m] * imag) * ampl_factor[Y];
			if (ncmp > 2)
				v[Z] += ampl[m] * (aka[Z][m] * real - akb[Z][m] * imag) * ampl_factor[Z];
		}
		if (verbose > 1)
			TurbGen_printf(FuncSig(__func__) + "exiting.\n");
	}; // get_turb_vector
	...
	void print_info(std::string print_mode)
	{
		...
		if (spect_form == 0)
			TurbGen_printf(" spectral form                                       = %i (Band)\n", spect_form);
		if (spect_form == 1)
		...
		if (spect_form == 2)
			TurbGen_printf(" spectral form                                       = "
				       ...
				       spect_form);
		if (spect_form == 2)
			TurbGen_printf(" power-law exponent                                  = %e\n", power_law_exp);
		if (spect_form == 2 && kmid != kmax)
			TurbGen_printf(" power-law exponent 2                                = %e\n", power_law_exp_2);
		if (spect_form == 2)
			TurbGen_printf(" power-law angles sampling exponent                  = %e\n", angles_exp);
		...
		if (spect_form == 2 && kmid != kmax)
			TurbGen_printf(" middle  wavenumber (in 2pi/Lx)                      = %e\n", kmid / (2 * M_PI) * L[X]);
	...
	// ******************************************************
      private:
	std::vector<double> read_from_parameter_file(std::string search, std::string type)
	{
		// ******************************************************
			...
			if (strncmp(line, search.c_str(), strlen(search.c_str())) == 0) {
				...
				if ((type == "2d") || (type == "3d")) {
					...
					if ((cnt != 0) && (cnt != ncmp - 1)) { // error check
						std::string msg = ClassSignature + "ERROR: parameter '%s' specifies %i components, which does not "
					...
					for (int d = 0; d < ncmp; d++) {
						...
						if (cnt == ncmp - 1) {				     // if user has specified ncmp components; else copy
												     // the same value into each component
							substr = cmpstr.substr(0, cmpstr.find(",")); // extract substring before first ','
							cmpstr = cmpstr.substr(cmpstr.find(",") + 1,
									       cmpstr.size() - cmpstr.find(",")); // reduce cmpstr to remainder for next
														  // loop iter
						}
						if (verbose > 1)
					...
					for (ik[Z] = -ikmax[Z]; ik[Z] <= ikmax[Z]; ik[Z]++) {
						...
						if ((ka >= kmin) && (ka <= kmax)) {
							...
							if (spect_form == 0)
								amplitude = 1.0; // Band
							if (spect_form == 1)
								amplitude = fabs(parab_prefact * pow(ka - kc, 2.0) + 1.0); // Parabola
		...
		if (spect_form == 2) {
			...
			for (ik[0] = ikmin[0]; ik[0] <= ikmax[0]; ik[0]++) {
				...
				for (iang = 1; iang <= nang; iang++) {
					...
					theta = M_PI / 2.0;
					if ((int)ndim > 2)
						theta = acos(1.0 - 2.0 * ran2(&seed)); // theta = [0,pi] sample the whole sphere
					...
					if ((ka >= kmin) && (ka <= kmax)) {
						...
						amplitude = sqrt(amplitude * pow((double)ik[0], (int)ndim - 1) / (double)(nang) * 4.0 * sqrt(3.0)) *
	...
	double ran2(int *idum)
	{
		...
		// ******************************************************
		static const int IM1 = 2147483563, IM2 = 2147483399, IMM1 = IM1 - 1, IA1 = 40014, IA2 = 40692, IQ1 = 53668, IQ2 = 52774, IR1 = 12211,
